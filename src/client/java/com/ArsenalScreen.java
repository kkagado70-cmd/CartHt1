...
public class ArsenalScreen extends Screen {
    ...
    public void render(GuiGraphics graphics, int mouseX, int mouseY, float delta) {
        ...
        int cardBg = ExampleModClient.isEnabled ? (isHovered ? BG_CARD_HOVER : 0xD5161E2C) : (isHovered ? BG_CARD_HOVER : BG_CARD);
        graphics.fill(cardX, cardY, cardX + cardW, cardY + cardH, cardBg);
        renderOutline(graphics, cardX, cardY, cardW, cardH, ExampleModClient.isEnabled ? ACCENT_RED : BORDER_COLOR);
        ...
        int bindY = cardY + 8;
        String bindText = isBindingKey ? "..." : (ExampleModClient.keybind != GLFW.GLFW_KEY_UNKNOWN && GLFW.glfwGetKeyName(ExampleModClient.keybind, 0) != null) ? GLFW.glfwGetKeyName(ExampleModClient.keybind, 0).toUpperCase() : "NONE";
        ...
        graphics.fill(switchX, switchY, switchX + switchW, switchY + switchH, ExampleModClient.isEnabled ? ACCENT_RED : 0x50000000);
        int toggleCircleX = ExampleModClient.isEnabled ? (switchX + switchW - 12) : (switchX + 2);
        ...
        graphics.drawString(this.font, "Aim Speed: " + (int) ExampleModClient.aimSpeed + " deg/tick", cardX + 12, optY, TEXT_MAIN, false);
        int sliderX = cardX + 160;
        int sliderW = 120;
        graphics.fill(sliderX, optY + 2, sliderX + sliderW, optY + 8, 0x60000000);
        int filledW = (int) (((ExampleModClient.aimSpeed - 5) / 55.0) * sliderW);
        ...
        renderCheckbox(graphics, cardX + 12, optY, "Auto Shoot Crossbow", ExampleModClient.autoShoot);
        optY += 22;

        renderCheckbox(graphics, cardX + 12, optY, "Swap Back to Previous Slot", ExampleModClient.swapBack);
    ...
    private void renderOutline(GuiGraphics graphics, int x, int y, int w, int h, int color) {
        ...
        graphics.fill(x, y + 1, x + w, y + h, color);
